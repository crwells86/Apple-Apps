...
func parseExpense(from transcript: String) -> Expense {
    ...
    let amount = parseAmount(from: transcript)
    let vendor = parseVendorOrCategory(from: transcript)
    
    guard let parsedAmount = amount else {
        return Expense(amount: 0, vendor: "Unknown", date: .now)
    }
    
    return Expense(amount: Decimal(parsedAmount), vendor: vendor, date: .now)
}
